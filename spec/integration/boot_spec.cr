...
LITIND_BIN = File.join(__DIR__, "../../build/litind")

module Litin::Integration
  describe "litind boot and IPC" do
    pending "litind binary not built — run `make` first" unless File.exists?(LITIND_BIN)

    # -------------------------------------------------------------------------
    # Helper: write a minimal service.sh into a temp service directory.
    # -------------------------------------------------------------------------
    def write_service(
      base_dir : String,
      name : String,
    ...
    def with_litind(& : IPC::Client ->)
      tmp = File.tempname("litin-integration")
